...
export default function AdminDashboard() {
  ...
  const renderOverview = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-gradient-to-r from-cyan-600 to-cyan-800 rounded-xl p-8 flex flex-col sm:flex-row items-center justify-between shadow-lg shadow-cyan-900/50 border border-cyan-500/50 group cursor-pointer" onClick={() => window.open('/', '_blank')}>
         <div>
            <h2 className="text-2xl font-black text-white flex items-center gap-3">
              <Cpu className="w-8 h-8 text-cyan-200" />
              NEXUS SWARM AI (Customer App)
            </h2>
            <p className="text-cyan-200 mt-2">Engage with Liya, Neo, and Maya in the full-screen immersive command center.</p>
         </div>
         <button className="mt-6 sm:mt-0 px-6 py-3 bg-white text-cyan-600 font-bold rounded-full hover:bg-cyan-100 transition-colors shadow-lg flex items-center gap-2 group-hover:scale-105 duration-300">
    ...
    <div className="bg-[#1c1d25] border border-slate-800/50 rounded-xl overflow-hidden flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-500 shadow-lg">
      <div className="p-5 flex justify-between items-center shrink-0">
        <h3 className="text-base font-bold text-white flex items-center gap-2">
           <Database className="w-5 h-5 text-cyan-500"/> SLT Customer Database (CRM)
        </h3>
        <span className="px-3 py-1 bg-cyan-500/10 text-cyan-500 rounded-lg text-xs font-bold border border-cyan-500/20">
              ...
              <tr key={i} className="hover:bg-slate-800/30 transition-colors">
                ...
                <td className="p-4">
                  <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded text-xs font-medium ${
                    ...
                    <span className={`w-2 h-2 rounded-full ${c.status === 'UP' ? 'bg-emerald-500' : 'bg-cyan-500'}`}></span>
                    {c.status}
                  </span>
                </td>
              </tr>
  ...
  const renderFullDB = () => {
    ...
    <div className="bg-[#1c1d25] border border-slate-800/50 rounded-xl overflow-hidden flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-500 shadow-lg">
      <div className="p-5 flex justify-between items-center shrink-0 border-b border-slate-800/50">
        <h3 className="text-base font-bold text-white flex items-center gap-2">
           <Server className="w-5 h-5 text-cyan-500"/> Full Dummy DB (200 Records)
        </h3>
        <span className="px-3 py-1 bg-cyan-500/10 text-cyan-500 rounded-lg text-xs font-bold border border-cyan-500/20">
          {filtered.length} Records Loaded
        </span>
            ...
            {filtered.map((c, i) => (
              <tr key={i} className="hover:bg-slate-800/30 transition-colors text-sm">
                ...
                <td className="p-4 text-xs text-slate-400">
                  {c.type === 'Copper' ? (
                     <span>SNR: <span className="text-emerald-400">{c.snr || 'N/A'}</span> | Attn: <span className="text-cyan-400">{c.attenuation || 'N/A'}</span></span>
                  ) : 'N/A'}
                </td>
                <td className="p-4 text-xs text-slate-400">
                   {c.type === 'Fiber' ? (
          ...
          <div key={i} className="bg-[#13141a] rounded-xl p-5 border border-slate-800 hover:border-cyan-500/30 transition-colors">
             <div className="flex items-center gap-4 mb-5">
                <div className="relative">
                  ...
                  <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-[#13141a] ${tech.status === 'Available' ? 'bg-emerald-500' : 'bg-cyan-500'}`}></div>
                </div>
                <div>
             ...
             <div className="flex justify-between items-end">
                <div>
                   ...
                   <p className={`text-sm font-medium ${tech.status === 'Available' ? 'text-emerald-500' : 'text-cyan-500'}`}>{tech.status}</p>
                </div>
          ...
          <div key={i} className="bg-[#1c1d25] rounded-xl p-5 border border-emerald-500/30 hover:border-emerald-500 transition-colors shadow-lg relative overflow-hidden">
             ...
             <div className="space-y-3 mt-5 relative z-10">
               <div className="flex justify-between items-center pb-2 border-b border-slate-800/50">
                 ...
                 <span className="text-sm font-medium text-cyan-400">{conn.pkg}</span>
               </div>
               <div className="flex justify-between items-center pb-2 border-b border-slate-800/50">
    ...
    <div className="flex h-screen bg-[#060913] text-slate-300 font-sans overflow-hidden">
      ...
      {activeTab !== 'ai-chat' && activeTab !== 'pathfinder-map' && (
        ...
        <div className="p-6 flex items-center gap-4">
           <div className="relative">
              <div className="w-12 h-12 rounded-full bg-slate-800 flex items-center justify-center border-2 border-cyan-500">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative hidden sm:block w-72">
               <input 
                 ...
                 className="w-full bg-[#13141a] border-none text-white text-sm rounded-full pl-5 pr-10 py-2.5 focus:outline-none focus:ring-1 focus:ring-cyan-500 transition-shadow"
               />
            ...
            <div className="flex-1 flex flex-col">
              ...
              {activeTab === 'pathfinder-map' && (
                <div className="w-full h-full flex flex-col relative">
                  <div className="p-3 bg-[#02040a] shrink-0 border-b border-slate-800/50">
                    <button 
                ...
                <div className="flex-1 min-h-full w-full flex flex-col bg-[#1c1d25] overflow-hidden shadow-lg">
                  ...
                  <div className="flex border-b border-slate-800/50 bg-[#13141a]">
                    ...
                    <button 
                      ...
                      className={`flex-1 py-3 text-sm font-bold transition-colors ${aiChatTab === 'liya' ? 'text-cyan-500 border-b-2 border-cyan-500' : 'text-slate-400 hover:text-white'}`}
                    >
                      Liya (Head of AI)
                    </button>
                    <button 
