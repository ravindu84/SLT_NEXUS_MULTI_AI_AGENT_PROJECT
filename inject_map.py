...
def inject_code():
    ...
    # 1. Replace the Cable component end
    cable_target = "  isRoad = false\n}: CableProps) {\n  // Generate curve/road-path points\n  const curvePoints = useMemo(() => {"
    
    # 2. Add TechnicianDispatchRoute before DigitalTwinSceneProps
...
interface DigitalTwinSceneProps {"""
    ...
    # 3. Inject where cables are rendered
    ...
    if target_interface in content and render_target in content:
        content = content.replace(target_interface, component_code)
        content = content.replace(render_target, render_injection)
